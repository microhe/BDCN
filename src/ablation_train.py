...
def adjust_learning_rate(optimizer, steps, step_size, gamma=0.1, logger=None):
    ...
    for param_group in optimizer.param_groups:
        param_group[
            'lr'] = param_group['lr'] * gamma  # (0.1 ** (steps // step_size))
        if logger:
...
def train(model, args):
    ...
    trainloader = torch.utils.data.DataLoader(
        train_img, batch_size=args.batch_size, shuffle=True, num_workers=5)
    ...
    for step in xrange(start_step, args.max_iter + 1):
        ...
        if step % args.display == 0:
            ...
            logger.info('iter: %d, lr: %e, loss: %f, time using: %f(%fs/iter)'
                        % (step, optimizer.param_groups[0]['lr'],
                           np.mean(mean_loss), tm, tm / args.display))
            start_time = time.time()
...
def parse_args():
    ...
    parser.add_argument(
        '-m', '--momentum', type=float, default=0.9, help='the momentum')
    parser.add_argument(
        '-c',
        '--cuda',
        action='store_true',
    ...
    parser.add_argument(
        ...
        default=10000,
        help=
        'the number of iters to decrease the learning rate, default is 10000')
    parser.add_argument(
        '--display',
        type=int,
        default=20,
